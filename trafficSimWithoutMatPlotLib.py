...
# traffic simulation
# alt version without MatPlotLib for graphs
...
class intersection:
    ...
    def controlNorm(self, time, chance):
        ...
        return (self.flow.getAvgWaitTime(0, [10, 10, 10, 10]) + self.flow.getAvgWaitTime(1, [10, 10, 10,
                                                                                             10]) + self.flow.getAvgWaitTime(
            2, [10, 10, 10, 10]) + self.flow.getAvgWaitTime(3, [10, 10, 10, 10])) / 4
...
def main():
    ...
    for i in range(10):  # run 10 1-hour simulations
        ...
        qIndex = 0
        normAvg[i] = intersectionSim.controlNorm(3600, [2, 3, 4,
                                                        3])  # ***Switch from Light vs Rush hour:  [2,3,4,3] vs [2,2,2,3]******
        longestQueue[0] = longestQueue[0] / longestQueue[2]

        intersectionSim1 = intersection()
        longestQueue[2] = 0
        qIndex = 1
        optAvg[i] = intersectionSim1.controlOptimized(3600, [2, 3, 4, 3])
    ...
    #plt.scatter(x2, optAvg, label=('Optimized Control, Avg: ' + str(optmean) + 's'), color="springgreen", marker="o",
     #           s=30)
    #plt.xlabel('Sim #')
